...
function getDayName(date) {
  ...
  let wString = '';
  if (d === 0) wString = 'Sunday';
  if (d === 1) wString = 'Monday';
  if (d === 2) wString = 'Tuesday';
  if (d === 3) wString = 'Wednesday';
  if (d === 4) wString = 'Thursday';
  if (d === 5) wString = 'Friday';
  if (d === 6) wString = 'Saturday';
  return wString;
}
...
function formatDate(date) {
  ...
  if (formattedDate.split(',')[1][1] === '0') {
    return `${formattedDate.split(',')[0]}, ${formattedDate.split(',')[1].slice(2)}`;
  }
...
function getWeekNumberByDate(date) {
  ...
  while (date.getFullYear() === day.getFullYear()) {
    if (date.getDay() === 0) {
      i += 1;
    }
...
function getNextFridayThe13th(date) {
  ...
  for (let i = month; i < 12; i += 1) {
    const next13th = new Date(year, i, 13);
  ...
  for (let i = 0; i < 12; i += 1) {
    const next13th = new Date(year + 1, i, 13);
    if (next13th.getDay() === 5) {
      return next13th;
    }
  }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  return arr.map((el) => el.slice(0, 10).split('-').reverse().join('-'));
}
